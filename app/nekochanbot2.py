...
db = None 
FIRESTORE_COLLECTION_NAME = "discord_tracked_original_vcs_prod_v3" 
...
async def register_new_vc_for_tracking(original_vc: discord.VoiceChannel, send_feedback_to_ctx=None):
    ...
    try:
            ...
            if guild_id_for_check and status_id_for_check:
                ...
                if guild_for_status_check:
                    ...
                    if isinstance(status_vc_obj, discord.VoiceChannel) and status_vc_obj.category and STATUS_CATEGORY_NAME.lower() in status_vc_obj.category.name.lower():
                        ...
                        return False # Exits critical section, lock released in finally
        ...
        if new_status_vc:
            ...
            if original_vc_id in vc_tracking: del vc_tracking[original_vc_id] # Should already be cleared by unregister if that path was taken
        ...
        if lock.locked():
            lock.release()
            logger.debug(f"[register_new_vc] Lock for VC ID: {original_vc_id} released in finally.")
...
async def unregister_vc_tracking(original_channel_id: int, guild: discord.Guild | None, send_feedback_to_ctx=None):
    ...
    try:
        ...
        if lock.locked():
            lock.release()
            logger.debug(f"[unregister_vc] Lock for VC ID: {original_channel_id} released in finally.")
...
async def unregister_vc_tracking_internal(original_channel_id: int, guild: discord.Guild | None, send_feedback_to_ctx=None, is_internal_call: bool = False):
    ...
    if track_info:
        ...
        # Use guild passed in if available, otherwise try to fetch from bot's cache
        ...
        if current_guild and status_channel_id:
            ...
            if status_vc and isinstance(status_vc, discord.VoiceChannel):
             ...
             logger.warning(f"Guild for original_channel_id {original_channel_id} not found. Cannot delete status VC {status_channel_id}.")
...
async def update_dynamic_status_channel_name(original_vc: discord.VoiceChannel, status_vc: discord.VoiceChannel):
    ...
    try:
        ...
        logger.error(f"[update_dynamic] Timeout acquiring lock for VC ID: {ovc_id}. Update skipped. Lock state before attempt: {'N/A - was not locked check'}")
    except Exception as e_outer_update:
        logger.error(f"[update_dynamic] Outer error for VC {ovc_id}: {e_outer_update}", exc_info=True)
    finally:
        if lock.locked():
            lock.release()
            logger.debug(f"[update_dynamic] Lock for VC ID: {ovc_id} released in finally.")
    ...
    for original_cid in tracked_ids_to_process:
        ...
        try:
            ...
            if isinstance(status_vc, discord.VoiceChannel) and status_vc.category and STATUS_CATEGORY_NAME.lower() in status_vc.category.name.lower():
                ...
                # update_dynamic_status_channel_name will attempt to acquire its own lock,
                # but since we are creating a task, it will run after this on_ready iteration for this VC releases its lock.
                ...
                # First, ensure it's fully unregistered from current state if it was partially tracked
                await unregister_vc_tracking_internal(original_cid, guild, is_internal_call=True) 
                
                # Now, attempt to create a new one. This is complex because register_new_vc_for_tracking also tries to lock.
                # For on_ready, it's better to call the core logic to avoid re-locking issues.
...
async def on_guild_channel_delete(channel: discord.abc.GuildChannel):
    ...
    if original_channel_id_to_process:
        ...
        async def handle_deletion_logic_wrapper(ocid_to_process, deleted_is_status, guild_where_deleted):
            ...
            try:
                ...
                if deleted_is_status:
                    original_vc_obj = guild_where_deleted.get_channel(ocid_to_process) if guild_where_deleted else None
                    if original_vc_obj and isinstance(original_vc_obj, discord.VoiceChannel):
                        ...
                        await unregister_vc_tracking_internal(ocid_to_process, guild_where_deleted, is_internal_call=True)
                ...
                if lock.locked():
                    lock.release()
                    logger.debug(f"[handle_deletion_logic_wrapper] Lock for {ocid_to_process} released in finally.")

        asyncio.create_task(handle_deletion_logic_wrapper(original_channel_id_to_process, is_status_vc_deleted, channel.guild), name=f"DeleteTask-{original_channel_id_to_process}")
        ...
        if isinstance(original_vc, discord.VoiceChannel) and isinstance(status_vc, discord.VoiceChannel):
            if status_vc.category is None or STATUS_CATEGORY_NAME.lower() not in status_vc.category.name.lower():
                ...
                async def fix_category_task_wrapper(ovc_obj, g_obj): 
                    ...
                    try:
                        ...
                        logger.error(f"[fix_category_task_wrapper] Timeout acquiring lock for {ovc_obj.id} during category fix (unregister part).")
                        return # Cannot proceed if lock not acquired for unregister
                    except Exception as e_fix_lock:
                        logger.error(f"[fix_category_task_wrapper] Error during lock acquire/unregister for {ovc_obj.id}: {e_fix_lock}", exc_info=True)
                        if lock_fix.locked(): lock_fix.release() # Ensure release on other exceptions too
                        return
                    finally:
                        if lock_fix.locked(): 
                            lock_fix.release()
                            logger.debug(f"[fix_category_task_wrapper] Lock for {ovc_obj.id} (unregister part) released in finally.")
                    ...
                    await register_new_vc_for_tracking(ovc_obj) # This will try to acquire lock again
            ...
            async def recreate_status_vc_task_wrapper(ovc_obj, g_obj): 
                ...
                try:
                    ...
                    logger.error(f"[recreate_status_vc_task_wrapper] Timeout acquiring lock for {ovc_obj.id} during status VC recreate (unregister part).")
                    return
                except Exception as e_recreate_lock:
                    logger.error(f"[recreate_status_vc_task_wrapper] Error during lock acquire/unregister for {ovc_obj.id}: {e_recreate_lock}", exc_info=True)
                    if lock_recreate.locked(): lock_recreate.release()
                    return
                finally:
                    if lock_recreate.locked(): 
                        lock_recreate.release()
                        logger.debug(f"[recreate_status_vc_task_wrapper] Lock for {ovc_obj.id} (unregister part) released in finally.")
...
async def nah_vc_command(ctx, *, channel_id_or_name: str):
    ...
    # These functions now handle their own locks and are designed to be called directly as tasks.
    # Send an initial acknowledgement to the user.
    await ctx.send(f"VC「{target_vc.name}」の追跡設定/解除処理を開始したニャ。完了まで少し待ってニャ。") # Initial ack

    if target_vc.id in vc_tracking:
        logger.info(f"コマンド: VC「{target_vc.name}」の追跡解除を試みます。")
        # Create a task so the command doesn't block, and pass ctx for potential final feedback
