...
# --- Logging Configuration (EARLY and EXPLICIT) ---
# This should be one of the VERY FIRST things your application does.
logging_configured_successfully = False
logging_setup_error_message = ""
try:
    import logging # Import logging here, after dotenv and sys if needed

    # Get the root logger.
    root_logger = logging.getLogger()
    root_logger.setLevel(logging.DEBUG) # Set root logger level to DEBUG

    # Remove any existing handlers from the root logger to prevent conflicts.
    if root_logger.hasHandlers():
        for handler in list(root_logger.handlers): # Iterate over a copy
            try:
                ...
                # Use print for critical errors if logging itself is failing
                print(f"ロギングハンドラ削除中にエラー: {e_handler_remove}", file=sys.stderr)


    # Create a stream handler explicitly using sys.stdout.
    ...
    logger_init_test = logging.getLogger(__name__) # Use a temporary logger for initial test
    
    # Test logging immediately after configuration.
    # These messages are crucial for diagnosing if logging is working on Render.
    logger_init_test.info(
        "ロギング設定が完了しました。ルートロガーレベル: %s, ハンドラレベル: %s, 実効レベル: %s",
        logging.getLevelName(root_logger.level),
        logging.getLevelName(stream_handler.level),
        logging.getLevelName(logger_init_test.getEffectiveLevel())
    )
    logger_init_test.debug("これはロギング設定直後のテストDEBUGメッセージです。これが表示されればDEBUG設定は有効です。")
    logging_configured_successfully = True
except Exception as e_logging_setup:
    # If logging setup fails, print to stderr as a last resort.
    logging_setup_error_message = f"ロギング設定中に重大なエラーが発生しました: {e_logging_setup}"
    print(logging_setup_error_message, file=sys.stderr)
# --- End of Logging Configuration ---

# Now import other modules that might also configure logging.
# Our explicit setup for the root logger should ideally take precedence.
import discord
...
# Get the main logger for the application *after* initial setup and test.
logger = logging.getLogger(__name__) # This logger will be used throughout the application.

if logging_configured_successfully:
    logger.info("メインのアプリケーションロガーが初期化されました。")
else:
    # If logging setup failed, this message might not appear in logs but will be printed to stderr.
    logger.error(f"メインのアプリケーションロガーの初期化に失敗しました。ロギング設定エラー: {logging_setup_error_message}")


# Optionally, control discord.py's logger level if it's too noisy or to see its debug logs.
# Do this *after* your own root logger setup.
logging.getLogger('discord').setLevel(logging.INFO) 
logging.getLogger('discord.http').setLevel(logging.WARNING) 
# You might want to set discord's level to DEBUG as well if you suspect issues there,
# but start with your application's logs first.
...
@app.route('/')
def home():
    logger.debug("Flask / endpoint called (Keep-alive)") # Add debug log for keep-alive
    return "I'm alive" 

def run_flask():
    port = int(os.environ.get('PORT', 8080)) 
    logger.info(f"Flaskサーバーを host=0.0.0.0, port={port} で起動します。")
    # Disable Werkzeug's default logger if it's interfering, or set its level.
...
def keep_alive():
    ...
    flask_thread.start()
    logger.info("Keep-aliveスレッドを開始しました。")


# ...(rest of the bot code, V5, remains the same from here)...
...
async def register_new_vc_for_tracking(original_vc: discord.VoiceChannel, send_feedback_to_ctx=None):
    ...
    try:
        ...
        if original_vc_id in vc_tracking:
            ...
            # ... (rest of the existing logic from V5 for checking if already tracking)
        ...
        if acquired and lock.locked(): # Only release if acquired
            lock.release()
        ...
        if current_guild and status_channel_id:
            ...
            if status_vc and isinstance(status_vc, discord.VoiceChannel):
                ...
                try:
                    ...
                    logger.error(f"[{task_name}|unregister_internal] Status VC {status_vc.name} (ID: {status_vc.id}) の削除に失敗 (権限不足): {e_forbidden}")
                except Exception as e_delete:
                    logger.error(f"[{task_name}|unregister_internal] Status VC {status_vc.name} (ID: {status_vc.id}) の削除中にエラー: {e_delete}", exc_info=True)
            elif status_vc:
                 logger.warning(f"[{task_name}|unregister_internal] Status Channel ID {status_channel_id} for original {original_channel_id} is not a VoiceChannel. Type: {type(status_vc)}")
            else:
                logger.info(f"[{task_name}|unregister_internal] Status VC ID {status_channel_id} for original {original_channel_id} not found in guild {current_guild.name}.")
        elif not current_guild and status_channel_id:
             logger.warning(f"[{task_name}|unregister_internal] Guild for original_channel_id {original_channel_id} not found. Cannot delete status VC {status_channel_id}.")
...
async def update_dynamic_status_channel_name(original_vc: discord.VoiceChannel, status_vc: discord.VoiceChannel):
    ...
    if lock.locked(): 
        logger.debug(f"[{task_name}|update_dynamic] Lock for VC ID {ovc_id} is ALREADY HELD by another task. Skipping this update cycle.")
        return
    ...
    try:
        ...
        except asyncio.TimeoutError: # ... (handle fetch timeout)
            logger.error(f"[{task_name}|update_dynamic] Timeout fetching status VC name for {status_vc.id}. Skipping.")
            return
        except discord.NotFound: # ... (handle fetch not found)
            logger.error(f"[{task_name}|update_dynamic] Status VC {status_vc.id} for {original_vc.name} not found. Periodic check should handle.")
            return
        except Exception as e_fetch: # ... (handle other fetch errors)
        ...
        if not can_update_by_bot_rule: # ... (bot rate limit skip)
            return 
        ...
        try:
            ...
            # ... (success logic for rate limit window and zero stats)
        ...
        except asyncio.TimeoutError: # ... (handle edit timeout)
             logger.error(f"[{task_name}|update_dynamic] Timeout editing status VC name for {status_vc.name} (ID: {status_vc.id}).")
        except discord.HTTPException as e_http: # ... (handle HTTP exceptions including 429)
        ...
        except Exception as e_edit: # ... (handle other edit errors)
            logger.error(f"[{task_name}|update_dynamic] Unexpected error editing {status_vc.name}: {e_edit}", exc_info=True)
    
    except asyncio.TimeoutError:
        logger.error(f"[{task_name}|update_dynamic] Timeout acquiring lock for VC ID: {ovc_id}. Update skipped.")
...
async def on_ready():
    ...
    for original_cid in tracked_ids_to_process:
        ...
        try:
            ...
            if original_cid not in vc_tracking: # Re-check after lock
            ...
            if not guild: # ... (handle guild not found)
            ...
            if not isinstance(original_vc, discord.VoiceChannel): # ... (handle original VC invalid)
            ...
            if isinstance(status_vc, discord.VoiceChannel) and status_vc.category and STATUS_CATEGORY_NAME.lower() in status_vc.category.name.lower():
                ...
                if status_vc: # ... (handle existing but invalid status VC)
                ...
                if new_status_vc_obj: # ... (recreate and save logic)
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    # ... (rest of the V5 logic for this event, unchanged)
...
async def on_guild_channel_delete(channel: discord.abc.GuildChannel):
    if not isinstance(channel, discord.VoiceChannel): return
    # ... (rest of the V5 logic for this event, specifically the handle_deletion_logic_wrapper, unchanged)
...
async def periodic_status_update():
    ...
    for original_cid in list(vc_tracking.keys()): 
        ...
        track_info = vc_tracking.get(original_cid)
        if not track_info: # ... (skip if not in tracking)
            continue

        guild = bot.get_guild(track_info["guild_id"])
        if not guild: # ... (schedule unregistration if guild not found)
        ...
        if isinstance(original_vc, discord.VoiceChannel) and isinstance(status_vc, discord.VoiceChannel):
            if status_vc.category is None or STATUS_CATEGORY_NAME.lower() not in status_vc.category.name.lower():
                # ... (schedule category fix task, same as V5)
                async def fix_category_task_wrapper(ovc_obj, g_obj): 
                    ...
                    try:
                        ...
                        logger.error(f"[{fix_task_name}|fix_category_task_wrapper] Timeout acquiring lock for {ovc_obj.id} (unregister part).")
                        return 
                    except Exception as e_fix_lock_unregister: # ... (handle other lock/unregister errors)
            ...
            # ... (schedule unregistration if original VC invalid)
            ...
            # ... (schedule status VC recreation task, same as V5)
            async def recreate_status_vc_task_wrapper(ovc_obj, g_obj): 
                ...
                try:
                    ...
                    return
                except Exception as e_recreate_lock_unregister: # ... (handle other lock/unregister errors)
...
async def nah_command(ctx, num: int):
    ...
    except discord.Forbidden: # ... (error handling)
        await ctx.send("メッセージを削除する権限がないニャ😿 (ボットの権限を確認してニャ)")
    except discord.HTTPException as e: # ... (error handling)
...
async def nah_command_error(ctx, error):
    if isinstance(error, commands.MissingPermissions): # ... (error handling)
        await ctx.send("このコマンドを実行する権限がニャいみたいだニャ… (メッセージ管理権限が必要だニャ)")
    elif isinstance(error, commands.BotMissingPermissions): # ... (error handling)
        await ctx.send("ボットにメッセージを削除する権限がないニャ😿 (ボットの権限を確認してニャ)")
    elif isinstance(error, commands.BadArgument): # ... (error handling)
...
async def nah_vc_command(ctx, *, channel_id_or_name: str):
    guild = ctx.guild
    if not guild: # ... (check guild)
        await ctx.send("このコマンドはサーバー内でのみ使用可能ですニャ🐈")
        return

    target_vc = None # ... (find target_vc by ID or name, same as V5)
    try: 
    ...
    if not target_vc or not isinstance(target_vc, discord.VoiceChannel): # ... (check target_vc validity)
        await ctx.send(f"指定された「{channel_id_or_name}」はボイスチャンネルとして見つからなかったニャ😿")
        return

    if target_vc.category and STATUS_CATEGORY_NAME.lower() in target_vc.category.name.lower(): # ... (check if target is status VC)
...
async def nah_vc_command_error(ctx, error):
    # ... (error handling for nah_vc_command, same as V5)
...
async def start_bot_main():
    ...
    if os.getenv("RENDER"): # Start keep-alive only if on Render
        keep_alive()
