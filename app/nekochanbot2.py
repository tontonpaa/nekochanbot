...
def run_flask():
    # RenderはPORT環境変数を注入するので、それを使用する
    # 設定されていなければデフォルトで8080などを使用（ローカルテスト用）
    port = int(os.environ.get('PORT', 8080))
    # 必ず host='0.0.0.0' を指定して、Renderがアクセスできるようにする
    app.run(host='0.0.0.0', port=port)

def keep_alive():
    """Flaskサーバーを別スレッドで起動する"""
    flask_thread = Thread(target=run_flask)
    # flask_thread.daemon = True # メインスレッド終了時にFlaskスレッドも終了させる場合
    flask_thread.start()
    logger.info(f"Keep-aliveサーバーがポート {os.environ.get('PORT', 8080)} で起動準備完了。")

# --- ロギング設定 ---
# (これはスクリプトの早い段階、理想的にはimport直後が良い)
...
# GOOGLE_APPLICATION_CREDENTIALS = os.getenv("GOOGLE_APPLICATION_CREDENTIALS") # init_firestore内でチェックされる

# ... (get_vc_lock, Firestoreヘルパー関数群, get_or_create_status_category, _create_status_vc_for_original, ...)
# ... (register_new_vc_for_tracking, unregister_vc_tracking, update_dynamic_status_channel_name, ...)
# ... (on_ready, on_voice_state_update, on_guild_channel_create, on_guild_channel_delete, periodic_status_update, ...)
# ... (Botコマンド nah_command, nah_vc_command, nah_help_prefix ...)
# 上記の関数群は変更なしでそのまま使用します。
...
async def periodic_status_update():
    ...
    for original_cid in list(vc_tracking.keys()):
        ...
        if not guild:
            ...
            await unregister_vc_tracking(original_cid, None) 
            continue
...
async def nah_vc_command(ctx, *, channel_id_or_name: str):
    ...
    if target_vc.id in vc_tracking:
        ...
        if success:
            ...
            # register_new_vc_for_tracking が False を返した場合で、既にフィードバックが送信されていないケース
            # (例: _create_status_vc_for_original で None が返るなど)
            is_already_tracking_message_sent = False
            if send_feedback_to_ctx and hasattr(send_feedback_to_ctx, 'message') and send_feedback_to_ctx.message:
                 # 簡易的なチェック：もし直前のメッセージが「既に追跡中」なら送らない
                async for msg in send_feedback_to_ctx.channel.history(limit=1):
...
async def start_bot_main():
    ...
    # GOOGLE_APPLICATION_CREDENTIALS のチェックは init_firestore 内で行われるため、ここでは省略可
    # if os.getenv("GOOGLE_APPLICATION_CREDENTIALS") is None:
    #     logger.warning("環境変数 GOOGLE_APPLICATION_CREDENTIALS が設定されていません。Firestoreは使用できません。")

    # Keep-aliveサーバーを起動 (Renderのヘルスチェックに応答するため)
    keep_alive() # これは同期関数で、内部でスレッドを開始する

    try:
        logger.info("Botの非同期処理を開始します...")
        # bot.start() は discord.py v2.0 以降で推奨される非同期起動方法
...
if __name__ == "__main__":
    # logging設定はグローバルスコープの上部で行うのが一般的だが、ここでも可
    # logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    # logger = logging.getLogger(__name__)

    # 環境変数の読み込み（スクリプト最上部で .env から読み込んでいる想定）
    # DISCORD_TOKEN = os.getenv("DISCORD_TOKEN") (これはグローバルスコープで取得済み)
    ...
    try:
        ...
        # asyncio.run() 自体のエラーや、start_bot_main でキャッチされなかった例外
